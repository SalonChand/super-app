                ...
                <div className="fixed inset-0 z-[100] bg-black flex flex-col justify-between">
                    ...
                    <div className="p-4 pt-6 bg-gradient-to-b from-black/80 to-transparent flex items-center justify-between z-10 absolute w-full top-0">
                        <div className="flex items-center gap-3">
                            <div className="w-10 h-10 rounded-full overflow-hidden bg-zinc-800">{viewingStory.profile_pic_url ? <img src={`${viewingStory.profile_pic_url}`} className="w-full h-full object-cover" /> : <User className="m-auto mt-2 text-zinc-500" />}</div>
                    ...
                    <div className="absolute inset-0 flex items-center justify-center -z-10">
                        ...
                        {viewingStory.media_type === 'video' ? <video src={`${viewingStory.media_url}`} className="w-full h-full object-cover" style={{ filter: viewingStory.filter_class }} autoPlay onTimeUpdate={handleVideoTimeUpdate} onEnded={goToNextStory} /> : <img src={`${viewingStory.media_url}`} className="w-full h-full object-cover" style={{ filter: viewingStory.filter_class }} />}
                    </div>
                ...
                {uniqueStories.map((story) => {
                    ...
                    return (
                        <div key={`story-${story.id}`} onClick={() => openStoryViewer(story.user_id)} className={`w-24 h-36 flex-shrink-0 rounded-2xl relative cursor-pointer overflow-hidden ring-2 ring-offset-2 ring-offset-black group transition-all duration-500 ${isSeen ? 'ring-zinc-600 opacity-60 hover:opacity-100' : 'ring-blue-500'}`}>
                            {story.media_type === 'video' ? <video src={`${story.media_url}`} className="absolute inset-0 w-full h-full object-cover opacity-80 group-hover:scale-105 transition-transform duration-500" style={{ filter: story.filter_class }} autoPlay muted loop /> : <img src={`${story.media_url}`} className="absolute inset-0 w-full h-full object-cover opacity-80 group-hover:scale-105 transition-transform duration-500" style={{ filter: story.filter_class }} />}
