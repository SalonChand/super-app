...
import { MessageCircle, Heart, Share, User, Send, Plus, X, Music, Type, Wand2, Eye, Paintbrush, Undo } from 'lucide-react';
...
function Feed() {
    ...
    useEffect(() => {
        if (showStoryEditor && draftFile && !draftFile.type.startsWith('video') && canvasRef.current) {
            ...
            // Make canvas match its CSS size perfectly so drawing is accurate
    ...
    // --- STORY UPLOAD LOGIC ---
    const startStoryDraft = (e) => { 
        const file = e.target.files[0]; 
        if (file) { setDraftFile(file); setDraftPreviewUrl(URL.createObjectURL(file)); setShowStoryEditor(true); setIsDrawingMode(false); } 
    };
    
    const closeStoryEditor = () => { 
        setShowStoryEditor(false); setDraftFile(null); setDraftPreviewUrl(null); setDraftCaption(''); setDraftFilter(STORY_FILTERS[0].value); setDraftSong(SONG_LIST[0]); 
    ...
    const uploadFinalStory = async () => {
        ...
        // 🔥 If they drew on the canvas, we must merge the image and the drawing!
        if (canvasRef.current && !draftFile.type.startsWith('video')) {
            // Create a temporary hidden canvas to merge them
            ...
            await new Promise(resolve => {
                img.onload = () => {
                    ...
                    // 1. Draw the base image (apply filter if selected)
                    mergeCtx.filter = draftFilter;
                    // Calculate object-fit cover math to match CSS preview
                    const scale = Math.max(mergeCanvas.width / img.width, mergeCanvas.height / img.height);
                    const x = (mergeCanvas.width / 2) - (img.width / 2) * scale;
                    const y = (mergeCanvas.height / 2) - (img.height / 2) * scale;
                    mergeCtx.drawImage(img, x, y, img.width * scale, img.height * scale);
                    
                    // 2. Draw their doodles over it
            ...
            // Convert merged canvas back into a File!
            const blob = await new Promise(resolve => mergeCanvas.toBlob(resolve, 'image/jpeg', 0.9));
            formData.append('media', blob, 'story.jpg');
            formData.append('filter_class', 'none'); // Filter is already permanently baked in now!
        } else {
            formData.append('media', draftFile);
            if (draftFilter !== 'none') formData.append('filter_class', draftFilter);
        }

        if (draftCaption.trim()) formData.append('caption', draftCaption); 
        if (draftSong !== 'No Music') formData.append('song_name', draftSong);
    ...
    return (
        <div className="w-full animate-fade-in pb-20 sm:pb-0 overflow-hidden relative">
            ...
            {viewingPostImage && (
                <div className="fixed inset-0 z-[120] bg-black/95 flex items-center justify-center animate-fade-in" onClick={() => setViewingPostImage(null)}>
                    ...
                    <img src={viewingPostImage} className="max-w-full max-h-full object-contain p-4" onClick={(e) => e.stopPropagation()} />
                </div>
            )}

            {/* 🔥 STORY EDITOR MODAL (NOW WITH DOODLE CANVAS) 🔥 */}
            {showStoryEditor && (
                <div className="fixed inset-0 z-[110] bg-zinc-950 flex flex-col">
                    ...
                    <div className="flex-1 bg-black relative flex items-center justify-center overflow-hidden">
                        {draftFile.type.startsWith('video') ? (
                            <video src={draftPreviewUrl} className="w-full h-full object-contain" style={{ filter: draftFilter }} autoPlay muted loop />
                        ) : (
                            <>
                                {/* Base Image */}
                                <img src={draftPreviewUrl} className="absolute inset-0 w-full h-full object-cover pointer-events-none" style={{ filter: draftFilter }} />
                                ...
                                <canvas 
                                    ...
                                    className={`absolute inset-0 w-full h-full z-20 ${isDrawingMode ? 'cursor-crosshair' : 'pointer-events-none'}`}
                                />
                            </>
                        )}
                    ...
                    <div className="bg-zinc-950 border-t border-zinc-800 p-4 space-y-4 relative z-30">
                        ...
                        {/* 🎨 DRAWING TOOLS (Only show for images) */}
                        {!draftFile.type.startsWith('video') && (
                            <div className="flex items-center gap-3 overflow-x-auto pb-2 border-b border-zinc-800" style={{ scrollbarWidth: 'none' }}>
                                <button 
                                    onClick={() => setIsDrawingMode(!isDrawingMode)} 
                                    className={`px-4 py-2 rounded-full text-sm font-bold flex items-center gap-2 flex-shrink-0 transition ${isDrawingMode ? 'bg-red-500 text-white shadow-lg shadow-red-500/40' : 'bg-zinc-800 text-zinc-300'}`}
                                >
                                    <Paintbrush size={16} /> {isDrawingMode ? "Drawing On" : "Draw"}
                                </button>
                                
                                {isDrawingMode && DRAW_COLORS.map(c => (
                                    <button key={c} onClick={() => setDrawColor(c)} className="w-8 h-8 rounded-full border-2 transition-transform flex-shrink-0 hover:scale-110" style={{ backgroundColor: c, borderColor: drawColor === c ? 'white' : 'transparent' }}></button>
                                ))}

                                {isDrawingMode && (
                                    <button onClick={clearCanvas} className="p-2 ml-auto text-zinc-400 hover:text-white transition bg-zinc-800 rounded-full"><Undo size={18}/></button>
                                )}
                            </div>
                        )}
                ...
                <div className="fixed inset-0 z-[100] bg-black flex flex-col justify-between">
                    ...
                    <div className="p-4 pt-6 bg-gradient-to-b from-black/80 to-transparent flex items-center justify-between z-10 absolute w-full top-0">
                        <div className="flex items-center gap-3">
                            <div className="w-10 h-10 rounded-full overflow-hidden bg-zinc-800">{viewingStory.profile_pic_url ? <img src={`${BACKEND_URL}${viewingStory.profile_pic_url}`} className="w-full h-full object-cover" /> : <User className="m-auto mt-2 text-zinc-500" />}</div>
                            <div><h3 className="text-white font-bold drop-shadow-md">{viewingStory.username}</h3><p className="text-xs text-zinc-300 drop-shadow-md">{formatTimeFriendly(viewingStory.created_at)}</p></div>
                        </div>
                    ...
                    <div className="absolute inset-0 flex items-center justify-center -z-10">
                        <div className="absolute inset-y-0 left-0 w-1/4 z-10 cursor-pointer" onClick={() => setViewingStory(null)}></div>
                        <div className="absolute inset-y-0 right-0 w-3/4 z-10 cursor-pointer" onClick={goToNextStory}></div>
                        {viewingStory.media_type === 'video' ? <video src={`${BACKEND_URL}${viewingStory.media_url}`} className="w-full h-full object-cover" style={{ filter: viewingStory.filter_class }} autoPlay onTimeUpdate={handleVideoTimeUpdate} onEnded={goToNextStory} /> : <img src={`${BACKEND_URL}${viewingStory.media_url}`} className="w-full h-full object-cover" style={{ filter: viewingStory.filter_class }} />}
                    </div>
                    <div className="absolute inset-0 pointer-events-none flex flex-col justify-between z-0 pb-28 pt-24">
                        <div className="flex justify-center">{viewingStory.song_name && <div className="bg-black/50 text-white text-sm font-bold py-1.5 px-4 rounded-full flex items-center gap-2 backdrop-blur-md"><Music size={14} className="text-pink-400" /> {viewingStory.song_name}</div>}</div>
                        <div className="flex justify-center px-4">{viewingStory.caption && <span className="bg-black/50 text-white text-2xl font-bold py-2 px-4 rounded-xl leading-tight backdrop-blur-md inline-block text-center mb-4">{viewingStory.caption}</span>}</div>
                    </div>
                    <div className="absolute bottom-0 w-full z-20">
                        {viewingStory.user_id == userId ? (
                            <div className="p-6 bg-gradient-to-t from-black via-black/80 to-transparent flex justify-center items-center gap-8"><div className="flex flex-col items-center"><Heart size={28} className="text-pink-500 fill-pink-500 mb-1 drop-shadow-md" /><span className="text-white font-bold">{viewingStory.like_count || 0} Likes</span></div><div className="flex flex-col items-center"><Eye size={28} className="text-blue-400 mb-1 drop-shadow-md" /><span className="text-white font-bold">{viewingStory.view_count || 0} Views</span></div></div>
                        ) : (
                            <div className="p-4 bg-gradient-to-t from-black/90 to-transparent flex gap-4 items-center">
                                <form onSubmit={(e) => handleStoryReply(e, viewingStory.user_id)} className="flex-1"><input type="text" value={storyReply} onChange={(e)=>setStoryReply(e.target.value)} placeholder={`Reply to ${viewingStory.username}...`} className="w-full bg-black/40 border border-zinc-500 rounded-full py-3 px-5 text-white placeholder-zinc-300 outline-none focus:border-white transition backdrop-blur-md" /></form>
                                <button onClick={() => handleStoryLike(viewingStory.id)} className="p-3"><Heart size={32} className={`transition-all hover:scale-110 ${viewingStory.user_liked ? 'fill-pink-500 text-pink-500 drop-shadow-[0_0_15px_rgba(236,72,153,0.8)]' : 'text-white drop-shadow-md'}`} /></button>
                            </div>
                        )}
                    </div>
                </div>
                ...
                {uniqueStories.map((story) => {
                    ...
                    return (
                        <div key={`story-${story.id}`} onClick={() => openStoryViewer(story.user_id)} className={`w-24 h-36 flex-shrink-0 rounded-2xl relative cursor-pointer overflow-hidden ring-2 ring-offset-2 ring-offset-black group transition-all duration-500 ${isSeen ? 'ring-zinc-600 opacity-60 hover:opacity-100' : 'ring-blue-500'}`}>
                            {story.media_type === 'video' ? <video src={`${BACKEND_URL}${story.media_url}`} className="absolute inset-0 w-full h-full object-cover opacity-80 group-hover:scale-105 transition-transform duration-500" style={{ filter: story.filter_class }} autoPlay muted loop /> : <img src={`${BACKEND_URL}${story.media_url}`} className="absolute inset-0 w-full h-full object-cover opacity-80 group-hover:scale-105 transition-transform duration-500" style={{ filter: story.filter_class }} />}
